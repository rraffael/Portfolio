...
- [x] Acessibilidade: **foco visível** (`:focus-visible`) em todos os interativos, **`aria-current`** no item de menu ativo (desktop + mobile), navegação por teclado nos cards (links nativos), e **`prefers-reduced-motion`** para as animações CSS (blink/bob/cookie/hover/estore) + scroll do deck instantâneo
- [x] **`prefers-reduced-motion` nas cenas de janela (SMIL)** — os 14 SVGs tiveram o SMIL convertido para **animações CSS** com um `<style>` interno + `@media (prefers-reduced-motion: reduce)` que **congela** o movimento (nuvem/chuva/neve/estrela/raio) mantendo a cena legível; vocabulário padronizado (`.drift`/`.twinkle`/`.rain`/`.snow`/`.flash`), motion em pixels inteiros pra manter `crispEdges`
- [x] **Acessibilidade extra**: `<html lang>` sincronizado com o locale (`_document.jsx` + efeito em `index.jsx`); `aria-haspopup`/`aria-expanded` no seletor de idioma e no menu hambúrguer (`aria-controls` → `#mobile-menu`)
